...
The files will be saved in the folder `projectconfigs/cluster-resource-quotas`
...
```
$ cat projectconfigs/cluster-resource-quotas/for-name.original
...
```
$ cat projectconfigs/cluster-resource-quotas/for-name.yaml
apiVersion: v1
kind: ResourceQuota
metadata:
  name: for-name
...
Once you decide the specific ResourceQuotas to apply to individual namespaces, copy the second file and change the quota values, the value for namespace (`namespace: CHANGEME`) and save the file. This changed quota would be applied to the target kubernetes cluster.
